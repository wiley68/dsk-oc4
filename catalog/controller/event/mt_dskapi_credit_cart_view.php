...
/**
 * Class MtDskapiCreditCartView
 *
 * @package Opencart\Catalog\Controller\Extension\MtDskapiCredit\Event
 */
class MtDskapiCreditCartView extends \Opencart\System\Engine\Controller
{
    ...
    /**
     * Constructor - initializes the controller and loads DSKAPI defaults
     *
    ...
    /**
     * Initializes the cart view event - adds DSKAPI widget to the cart page
     *
     * @param string &$route
     * @param array &$data
     * @param string &$output
    ...
    public function init(&$route, &$data, &$output): void
    {
        ...
        $dskapi_price = $this->cart->getSubTotal() + array_sum($this->cart->getTaxes());
        ...
        $data['current_currency'] = $dskapi_currency_code;
        ...
        $data['dskapi_gap'] = $this->config->get($this->module . '_gap');
        $data['dskapi_gap_popup'] = $this->config->get($this->module . '_gap_popup');
        ...
        if ($position_hook1 !== false) {
            ...
            if ($position_hook2_in_suboutput !== false) {
                ...
                $output = substr($output, 0, $position_hook2_after) . $this->load->view($this->path . '_cart', $data) . substr($output, $position_hook2_after);
            }
        }
    }
