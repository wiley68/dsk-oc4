    ...
    if (this.readyState == 4) {
      ...
      if (dsk_is_visible) {
        if (options) {
          ...
          const dskapi_vnoski_button = document.getElementById(
            "dskapi_vnoski_button"
          );
          const dskapi_vnoska_button = document.getElementById(
            "dskapi_vnoska_button"
          );
...
/**
 * Наблюдава AJAX обновявания на количката и след завършване прави soft reload,
 * за да се опреснят сумите и попъпа без да поддържаме ръчно динамични изчисления.
 */
function dskapi_setupCartReloadOnAjax() {
  if (window.dskapiCartReloadPatched) {
    return;
  }
  window.dskapiCartReloadPatched = true;

  const shouldWatchUrl = (url) => {
    if (typeof url !== "string") {
      return false;
    }
    return (
      url.includes("route=checkout/cart") ||
  ...
  XMLHttpRequest.prototype.open = function (method, url) {
    this.__dskapi_url = url;
    return originalOpen.apply(this, arguments);
  };
  XMLHttpRequest.prototype.send = function (body) {
    this.addEventListener("load", () => {
      if (shouldWatchUrl(this.__dskapi_url)) {
        scheduleReload();
      }
    });
